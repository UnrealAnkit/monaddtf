// Live protocol stats — NAV/share, TVL and supply read from the vault contract.

import { PRICE_DECIMALS, SHARE_DECIMALS, SHARE_SYMBOL } from "@/lib/config";
import { fmtUnits } from "@/lib/folio";
import type { NavInfo } from "@/lib/folio";
...
export function Stats({ nav, supply }: { nav: NavInfo | null; supply: bigint | null }) {
  const stats = [
    ...
    },
    {
      ...
      live: supply !== null,
      note: `${SHARE_SYMBOL} shares minted`,
    },
  ];
  ...
  return (
    <Reveal className="mt-20">
      ...
      <p className="mt-2 text-right text-[11px] text-gray-400">
        READ LIVE FROM THE VAULT CONTRACT · REFRESHES EVERY 15S
      </p>
    </Reveal>
  );
}
