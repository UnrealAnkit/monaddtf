...
const ITEMS: NavCard[] = [
  {
    ...
    links: [
      ...
      { label: "Why Alloy", to: "why", ariaLabel: "Jump to why Alloy" },
      { label: "About & FAQ", href: "/about", ariaLabel: "About Alloy and frequently asked questions" },
    ],
  },
  {
    label: "Launch",
    ...
    links: [
      ...
      ...(EXPLORER_URL
        ? [{ label: "View on Explorer", href: `${EXPLORER_URL}/address/${VAULT_ADDRESS}`, ariaLabel: "View the vault on the block explorer", external: true }]
        : []),
    ],
  },
];
...
export function Navbar() {
  ...
  const createTimeline = () => {
    ...
    tl.to(cardsRef.current, { y: 0, opacity: 1, duration: 0.4, ease: EASE, stagger: 0.08 }, "-=0.1");
    return tl;
  };
      ...
      >
        ...
        >
          {ITEMS.map((item, idx) => (
            ...
            >
              <div className="text-[18px] font-medium tracking-[-0.5px] md:text-[20px]">{item.label}</div>
              <div className="mt-auto flex flex-col gap-[2px]">
                {item.links.map((lnk) => {
                  ...
                  const cls = "inline-flex items-center gap-1.5 text-[15px] transition-opacity duration-300 hover:opacity-75";
                  if (lnk.to) {
                    return (
                  ...
                  if (lnk.external) {
                    return (
                      <a key={lnk.label} href={lnk.href} target="_blank" rel="noopener noreferrer" aria-label={lnk.ariaLabel} className={cls}>
                        {inner}
                      </a>
                    );
                  }
