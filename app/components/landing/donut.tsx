...
import { SHARE_SYMBOL } from "@/lib/config";
  ...
  return (
    <div className="relative mx-auto h-64 w-64">
      <svg viewBox="0 0 100 100" className="h-full w-full" role="img" aria-label="Basket composition">
        {slices.map((s, i) => {
          const offset = slices.slice(0, i).reduce((sum, slice) => sum + slice.frac * C, 0);
          const seg = (
            <circle
              key={i}
              cx="50"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <div className="font-heading text-3xl font-bold">{SHARE_SYMBOL}</div>
